...
economic_projections <- readxl::read_xlsx('inst/extdata/projections.xlsx', sheet = 'economic') %>% #Reading in data from the CBO economic projection (they have haver codes corresponding to cbo update)
...
budget_projections <- readxl::read_xlsx('inst/extdata/projections.xlsx', sheet = 'budget') %>% 
  ...
  fiscal_to_calendar() %>% #changing fiscal year to calendar year by taking it back 1 quarter (cbo definiton)
  
 ##Not sure what's going on here -- could we go over timing in general?
    mutate(
  #federal_ui_timing = case_when(date <= yearquarter('2020 Q4') ~ 0,
...
#Creating cares and crrca specific social benefits and subsidies (OLD)
...
projections <- 
  budget_projections %>% 
  left_join(economic_projections, by = 'date') %>% #TODO: rename this as period 
